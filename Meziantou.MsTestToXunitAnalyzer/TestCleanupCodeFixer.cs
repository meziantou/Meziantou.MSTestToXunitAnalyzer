...
public sealed class TestCleanupCodeFixer : CodeFixProvider
{
    public override FixAllProvider GetFixAllProvider() => WellKnownFixAllProviders.BatchFixer;

    public override ImmutableArray<string> FixableDiagnosticIds { get; } = ImmutableArray.Create("MSTestXunit208");
    ...
    private static async Task<Document> Rewrite(Document document, SyntaxNode nodeToFix, CancellationToken cancellationToken)
    {
        ...
        var method = (MethodDeclarationSyntax)operation.Syntax.AncestorsAndSelf().FirstOrDefault(node => node.IsKind(SyntaxKind.MethodDeclaration));
        if (method is null)
            return document;
        ...
        var symbol = editor.SemanticModel.GetDeclaredSymbol(typeDeclaration);
        if (symbol is null)
            return document;

        if (!symbol.IsOrImplements(types.IDisposableSymbol))
        {
            editor.AddInterfaceType(typeDeclaration, generator.TypeExpression(types.IDisposableSymbol));
        }

        var ctor = generator.MethodDeclaration("Dispose",
